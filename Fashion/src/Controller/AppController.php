...
class AppController extends Controller
{
    ...
    private function setInit()
    {
        ...
        $this->loadComponent('TinyAuth.Auth', [
            ...
            'authError' => __('Did you really think you are allowed to do that?'),
            'loginRedirect' => [
                'controller' => 'Carts',
                'action' => 'index'
            ],
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $pages = $this->Pages->find('List', ['keyField' => 'slug','valueField' => 'title'])->where(['status' => 1]);
        $categories = $this->Products->ProductCategories->find('all')->where(['ProductCategories.status' => 1,'ProductCategories.parent_id' => 1]);
