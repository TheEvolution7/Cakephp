        ...
        <div class="row align-items-center mb-4">
                    ...
                    <div class="btn-group">
                        <?= $this->Html->link('<i class="fa fa-plus"></i>' . __d('acp','Add New'), 
                            [
                                'action' => 'add',
                                '?' => [
                                    'album_category_id' => !empty($requestQuery['album_category_id']) ? $requestQuery['album_category_id'] : null
                                ]
                            ],
                            [
            ...
            <div class="col-md-6 my-3">
                <form class="d-flex justify-content-end align-items-center d-table mx-auto w-auto" action="">
                    ...
                    <label class="mr-1 mb-0">
                        <?= $this->Form->control('album_category_id', [
                            ...
                            'value' => !empty($requestQuery['album_category_id']) ? $requestQuery['album_category_id'] : null,
                            ]);
                        ?>
                    </label>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php foreach ($albums as $album): ?>
                                <tr data-id="<?= $album->id?>" class="text-center">
                                    <th scope="row"><?= $this->Number->format($album->id) ?></th>
                                    <td>
                                        <?php
                                            $width = 0;
                                            $height = 0;
                                        ...
                                        ?>
                                        <a href="<?= $this->Url->build(['action' => 'edit', $album->id]) ?>"><img src="<?php echo $this->Url->build(DS . $imagePath) ?>" class="thumbnail _cover" data-skin="dark" data-toggle="kt-tooltip" title="" data-html="true" data-original-title="<?= "File Information <br>Width: " . $width . "px <br>Height: " . $height . "px <br>Extension: ". $extension ?>"></a>
                                    </td>
                                    <td>
                                        <div style="width: 100px"><?= h($this->Text->truncate($album->title, 120, ['ellipsis' => ' ...', 'exact' => false ])) ?></div>
                                            
                                    </td>
                                    
                                    <td>
                                        <div style="text-align: left;">
                                        <?php foreach ($album->album_categories as $key => $cat) {
                                            if (isset($cat->id)) {
                                                echo '<a href="'. $this->Url->build(['controller' => 'Albums', 'action' => 'index', 'album_category_id' => $cat->id]) .'"><button type="button" class="btn btn-bold btn-sm btn-font-sm btn-label-success mb-2" >'. $cat->title .'</button></a>';
                                            }
                                            
                                        }?>
                                        </div>
                                    </td>
                                    <td><?= $this->Element('Acp.Core' . DS . 'language', ['id' => $album->id, 'translations' => $album->_translations]) ?></td>
                                    <td> 
                                    ...
                                    <td class="width-sort sort"><?php echo $this->Form->number('sort', ['div' => false, 'label' => false, 'class' => 'form-control input-small', 'value' => $album->sort,'style' => 'margin: auto !important;']); ?></td>
                                    <td><?= $this->Element('Acp.Core' . DS . 'action', ['id' => $album->id]) ?></td>
                                    <td>
                                        <label class="kt-checkbox kt-checkbox--single kt-checkbox--solid no-mg">
