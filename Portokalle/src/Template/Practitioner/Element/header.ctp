<div class="header2 js-header2">
    ...
    <a class="header2__logo" href="#"><img class="header2__pic header2__pic_black" src="<?= $webroot ?>/img/logo.svg" alt="" /></a>
    <div class="header2__search">
        <?= $this->Form->create(null, ['type' => 'get', 'url' => ['controller' => 'Appointments', 'action' => 'index']]) ?>
        <button class="header2__open">
            <svg class="icon icon-search">
                <use xlink:href="<?= $webroot ?>img/sprite.svg#icon-search"></use>
            </svg>
        </button>
        <input class="header2__input" type="text" placeholder="Search" name="keywords" />
        <?= $this->Form->end() ?>
    </div>
    ...
    ?>
    <a class="header2__profile" href="<?= $this->Url->build(['controller' => 'Dashboards', 'action' => 'index']) ?>"><img class="header2__pic" src="<?php echo $this->Url->build($image); ?>"/></a>
