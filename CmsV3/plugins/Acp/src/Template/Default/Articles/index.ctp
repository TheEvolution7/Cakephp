                    ...
                    <div class="btn-group">
                        <?= $this->Html->link('<i class="fa fa-plus"></i>' . __d('acp','Add New'), 
                            [
                                'action' => 'add',
                                '?' => [
                                    'article_category_id' => !empty($requestQuery['article_category_id']) ? $requestQuery['article_category_id'] : null
                                ]
                            ],
                            [
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php foreach ($articles as $article): ?>
                                <tr data-id="<?= $article->id?>" class="text-center">
                                    <th scope="row"><?= $this->Number->format($article->id) ?></th>
                                    <td>
                                        <?php
                                            $width = 0;
                                            $height = 0;
                                            ...
                                            if (!empty($article->image)) {
                                                ...
                                                if ($width > 500) {
                                                    $imagePath = 'uploads' . DS . 'articles' . DS . $article->id . DS . 'thumbnail' . DS . $article->image;
                                                }
                                            }
                                        ?>
                                        <a href="<?= $this->Url->build(['action' => 'edit', $article->id]) ?>"><img src="<?php echo $this->Url->build(DS . $imagePath) ?>" class="thumbnail _cover" data-skin="dark" data-toggle="kt-tooltip" title="" data-html="true" data-original-title="<?= "File Information <br>Width: " . $width . "px <br>Height: " . $height . "px <br>Extension: ". $extension ?>"></a>
                                    </td>
                                    <td>
                                        <div style="width: 100px"><?= h($this->Text->truncate($article->title, 120, ['ellipsis' => ' ...', 'exact' => false ])) ?></div>
                                            
                                    </td>
                                    
                                    <td>
                                        <div>
                                        <?php foreach ($article->article_categories as $key => $cat) {
                                            if (isset($cat->id)) {
                                                echo '<a href="'. $this->Url->build(['controller' => 'articles', 'action' => 'index', 'article_category_id' => $cat->id]) .'"><button type="button" class="btn btn-bold btn-sm btn-font-sm btn-label-success mb-2" >'. $cat->title .'</button></a>';
                                            }
                                            
                                        }?>
                                        </div>
                                    </td>
                                    <td><?= $this->Element('Acp.Core' . DS . 'language', ['id' => $article->id, 'translations' => $article->_translations]) ?></td>
                                    <td> 
                                    ...
                                    <td class="width-sort sort"><?php echo $this->Form->number('sort', ['div' => false, 'label' => false, 'class' => 'form-control input-small', 'value' => $article->sort,'style' => 'margin: auto !important;']); ?></td>
                                    <td><?= $this->Element('Acp.Core' . DS . 'action', ['id' => $article->id]) ?></td>
                                    <td>
                                        <label class="kt-checkbox kt-checkbox--single kt-checkbox--solid no-mg">
