...
class UploadBehavior extends Behavior
{
    ...
    public function beforeSave(Event $event, Entity $entity, ArrayObject $options)
    {
     ...
     /**
     * Clone from beforeSave and custom by Ngoc
     * in the database too.
     *
     * @param \Cake\Event\Event $event The beforeSave event that was fired
     * @param \ArrayObject $configs
     * @return void|false
     */
    public function uploadFile(Entity $entity, $configs)
    {
        $response = [];
        foreach ($configs as $field => $settings) {
            ...
            $response[] = [
                'field' => $field,
                'name' => $filename, 
                Hash::get($settings, 'fields.dir', 'dir') => $basepath,
                Hash::get($settings, 'fields.size', 'size') => $data['size'],
                Hash::get($settings, 'fields.type', 'type') => $data['type']
            ];
        }
     ...
     * @return void|false
     */
    public function deleteFile(Entity $entity, $configs, $file = null)
    {
        $result = true;
        ...
        foreach ($configs as $field => $settings) {
            ...
            // Custom
            $deleteFolder = Hash::get($settings, 'deleteFolder', null);
            $writer = $this->getWriter($entity, [], $field, $settings);
            if ($deleteFolder) {
                $success = $writer->deleteDir($path);
            } else {
    ...
    public function afterDelete(Event $event, Entity $entity, ArrayObject $options)
    {
        ...
        if ($entity->get('uuid')) {
            foreach ($this->getConfig(null, []) as $field => $settings) {
                ...
                $writer = $this->getWriter($entity, [], $field, $settings);
                if ($deleteFolder) {
                    $success = $writer->deleteDir(str_replace($field . DS, '', $path));
                } else {
                    $callback = Hash::get($settings, 'deleteCallback', null);
                    if ($callback && is_callable($callback)) {
                        $files = $callback($path, $entity, $field, $settings);
