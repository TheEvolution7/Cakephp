                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php foreach ($attributes as $attribute): ?>
                                <tr data-id="<?= $attribute->id?>" class="text-center">
                                    <th scope="row"><?= $this->Number->format($attribute->id) ?></th>
                                    <td><?= h($this->Text->truncate($attribute->title, 120, ['ellipsis' => ' ...', 'exact' => false ])) ?></td>
                                    <td><a class="btn btn-bold btn-sm btn-font-sm btn-label-info mb-2" href="<?= $this->Url->build(['plugin' => 'Acp', 'controller' => 'AttributeValues', 'action' => 'index',$attribute->id]) ?>"><?php echo __d('acp', 'Change values') ?></a></td>
                                    ...
                                    <td><?= $this->Element('Acp.Core' . DS . 'language', ['id' => $attribute->id, 'translations' => $attribute->_translations]) ?></td>
                                    
                                    <td class="width-sort sort"><?php echo $this->Form->number('sort', ['div' => false, 'label' => false, 'class' => 'form-control input-small', 'value' => $attribute->sort,'style' => 'margin: auto !important;']); ?></td>
                                    <td><?= $this->Element('Acp.Core' . DS . 'action', ['id' => $attribute->id]) ?></td>
                                    <td>
                                        <label class="kt-checkbox kt-checkbox--single kt-checkbox--solid no-mg">
