...
class AppController extends BaseController
{
    ...
    public function jGetImage($id = null){
        ...
        if ($this->request->is('post', 'ajax')) {
            ...
            $this->loadModel('Acp.ProductImages');

            $productImagesTable = TableRegistry::get('Acp.ProductImages');
            $productImagesTable = TableRegistry::getTableLocator()->get('Acp.ProductImages');

            $data = $productImagesTable->get($id);
            $response = [
                'status' => 1,
    ...
    public function jSaveImage(){
        ...
        if ($this->request->is('post', 'ajax')) {
            ...
            $product_image = $this->ProductImages->newEntity();
            $product_image = $this->ProductImages->patchEntity($product_image, $requestData);
            $this->ProductImages->save($product_image);
            $response = [
                'status' => 1,
                'message' => __d('acp', 'Success'),
            ];  
            ...
            if (!empty($modelName) && !empty($requestData)) {
                ...
                if ($this->$modelName->behaviors()->has('Upload')) {
                    ...
                    if ($modelName == 'Products') {
                        $product_detail = $this->Products->find('all',[
                            'conditions' => [
                                'Products.id' => $requestData['id']
                            ]
                        ])->first();
                        if (!empty($product_detail)) {
                            $this->loadModel('Acp.ProductImages');
                            $product_image = $this->ProductImages->newEntity();
                            $product_image->product_id = $requestData['parent_id'];
                            $product_image->title = $product_detail->title;
                            $product_image->description = $product_detail->description; 
                            $this->ProductImages->save($product_image);

                            $productImagesTable = TableRegistry::get('ProductImages');
                            $productImagesTable = TableRegistry::getTableLocator()->get('ProductImages');

                            $save_image = $productImagesTable->get($product_image->id);

                            $save_image->image = $files[0]['name'];
                            $productImagesTable->save($save_image); 
                        }
                    }
                    // if ($modelName == 'Albums') {
