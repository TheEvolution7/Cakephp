        ...
        <div class="kt-portlet__body">
            <ul id="image-list" >
                ...
                <?php foreach ($images as $key => $image) {
                    ...
                    $info = $this->Cms->getInfoImage('uploads' . DS . 'articles' . DS . $article->id, $image);
                    echo '<li id="image_' . $key . '" >
                        <img src="' . $this->Cms->processImagePath('uploads' . DS . 'articles' . DS . $article->id, $image) . '" alt="' . $image . '" data-skin="dark" data-toggle="kt-tooltip" title="" data-html="true" data-original-title="'. "File Information <br>Width: " . $info['width'] . "px <br>Height: " . $info['height'] . "px <br>Extension: ". $info['extension'] .'"></li>';
                } ?>
            </ul>
                ...
                <div class="col-md-7 col-sm-9 col-12">
                    <div class="kt-portlet__foot ta-foot">
                        <div class="kt-form__actions">
                            <button type="button" onclick="location.href='<?= $this->Url->build(['controller' => 'Articles', 'action' => 'edit', $article->id]) ?>';" class="btn btn-success btn-icon-text"><?php echo __d('acp', 'Back to this article') ?></button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function () {
            ...
            $("#image-list").sortable({
                ...
                if (imageIdsArray.length != 1) {
                    $.ajax({
                        url: '<?= $this->Url->build(['controller' => 'Articles', 'action' => 'reorderImage', $article->id]) ?>',
                        type: 'post',
