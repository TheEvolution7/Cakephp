                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php foreach ($currencies as $currency): ?>
                                <tr data-id="<?= $currency->id?>" class="text-center">
                                    <th scope="row"><?= $currency->id ?></th>
                                    <td>
                                        <?php 
                                            if (!empty($currency->image)) {
                                                $imagePath = 'uploads' . DS . 'currencies' . DS . $currency->id . DS . $currency->image;
                                            } else {
                                                $imagePath = 'img/no_thumb.png';
                                            }
                                        ?>
                                        <a href="<?= $this->Url->build(['action' => 'edit', $currency->id]) ?>"><img src="<?php echo $this->Url->build(DS . $imagePath) ?>" class="thumbnail _cover"></a>
                                    </td>
                                    <td><?= h($this->Text->truncate($currency->title, 120, ['ellipsis' => ' ...', 'exact' => false ])) ?></td>
                                    ...
                                    <td><?= $this->Element('Acp.Core' . DS . 'action', ['id' => $currency->id]) ?></td>
                                    <td>
                                        <label class="kt-checkbox kt-checkbox--single kt-checkbox--solid no-mg">
