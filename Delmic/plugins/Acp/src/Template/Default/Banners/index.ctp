        ...
        <div class="row align-items-center mb-4">
                    ...
                    <div class="btn-group">
                        <?= $this->Html->link('<i class="fa fa-plus"></i>' . __d('acp','Add New'), 
                            [
                                'action' => 'add',
                                '?' => [
                                    'banner_category_id' => !empty($requestQuery['banner_category_id']) ? $requestQuery['banner_category_id'] : null
                                ]
                            ],
                            [
            ...
            <div class="col-md-6 my-3">
                <form class="d-flex justify-content-end align-items-center d-table mx-auto w-auto" action="">
                    ...
                    <label class="mr-1 mb-0">
                        <?= $this->Form->control('banner_category_id', [
                            ...
                            'value' => !empty($requestQuery['banner_category_id']) ? $requestQuery['banner_category_id'] : null,
                            ]);
                        ?>
                    </label>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php foreach ($banners as $banner): ?>
                                <tr data-id="<?= $banner->id?>" class="text-center">
                                    <th scope="row"><?= $this->Number->format($banner->id) ?></th>
                                    <td>
                                        <?php
                                            $width = 0;
                                            $height = 0;
                                            ...
                                            if (!empty($banner->image)) {
                                                ...
                                                $imagePath = 'uploads' . DS . 'banners' . DS . $banner->id . DS . $banner->image;
                                                ...
                                                if ($width > 500) {
                                                    $imagePath = 'uploads' . DS . 'banners' . DS . $banner->id . DS . 'thumbnail' . DS . $banner->image;
                                                }
                                            }
                                        ?>
                                        <a href="<?= $this->Url->build(['action' => 'edit', $banner->id]) ?>"><img src="<?php echo $this->Url->build(DS . $imagePath) ?>" class="thumbnail _cover" data-skin="dark" data-toggle="kt-tooltip" title="" data-html="true" data-original-title="<?= "File Information <br>Width: " . $width . "px <br>Height: " . $height . "px <br>Extension: ". $extension ?>"></a>
                                    </td>
                                    <td>
                                        <div style="width: 100px"><?= h($this->Text->truncate($banner->title, 120, ['ellipsis' => ' ...', 'exact' => false ])) ?></div>
                                            
                                    </td>
                                    
                                    <td><a href="<?= $this->Url->build(['controller' => 'Banners', 'action' => 'index', 'banner_category_id' => $banner->banner_category->id]) ?>"><button type="button" class="btn btn-bold btn-sm btn-font-sm btn-label-success" ><?= $banner->banner_category->title ?></button></a></td>
                                    <td><?= $this->Element('Acp.Core' . DS . 'language', ['id' => $banner->id, 'translations' => $banner->_translations]) ?></td>
                                    <td> 
                                    ...
                                    <td class="width-sort sort"><?php echo $this->Form->number('sort', ['div' => false, 'label' => false, 'class' => 'form-control input-small', 'value' => $banner->sort,'style' => 'margin: auto !important;']); ?></td>
                                    <td><?= $this->Element('Acp.Core' . DS . 'action', ['id' => $banner->id]) ?></td>
                                    <td>
                                        <label class="kt-checkbox kt-checkbox--single kt-checkbox--solid no-mg">
