...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__d('acp', 'Edit Document Category'), ['action' => 'edit', $document->id]) ?> </li>
        <li><?= $this->Form->postLink(__d('acp', 'Delete Document Category'), ['action' => 'delete', $document->id], ['confirm' => __d('acp', 'Are you sure you want to delete # {0}?', $document->id)]) ?> </li>
...
<div class="documentCategories view large-9 medium-8 columns content">
    ...
    <table class="vertical-table">
        <tr>
            ...
            <td><?= $document->has('parent_document_category') ? $this->Html->link($document->parent_document_category->id, ['controller' => 'DocumentCategories', 'action' => 'view', $document->parent_document_category->id]) : '' ?></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><?= $document->status ? __d('acp', 'Yes') : __d('acp', 'No'); ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __d('acp', 'Related Document Categories') ?></h4>
        <?php if (!empty($document->child_document_categories)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($document->child_document_categories as $childDocumentCategories): ?>
            <tr>
