(function ($) {
    ...
    //     // Learn 
    //     function learnProgressBar(progressBar, value) {
    //         value = Math.round(value);
    //         progressBar.querySelector(".progress-fill").style.width = `${value}%`;
    //         progressBar.querySelector(".progress-fill").textContent = `${value}%`;
    //     }

    //     const myProgressBar = document.querySelector(".progress");

    //     /* Example */
    //     learnProgressBar(myProgressBar, 32);
    // });
    ...
    $('.list-question').each(function () {
        ...
        function checkQuiz() {
            ...
            // rightQuestions.innerHTML = rightAnswer;
            // wrongQuestions.innerHTML = (questions.length - rightAnswer);
    ...
    $('.collapse-section').each(function(){
        ...
        var $this = $(this)
        $('.btn-link-collapse', this).on('click', function(){
            $('.collapse-content', $this).slideToggle();
            $(this).toggleClass('show')
        })
    })
    $(document).on('click', '.section-question a[href^="#"]', function (event) {
        event.preventDefault();
    
        $('html, body').animate({
            scrollTop: $($.attr(this, 'href')).offset().top - ( $('.header-inner').height() + 80 )
        }, 500);
    });

    var nameArray = [];
    $('.section-question li').each(function(){
        nameArray.push($('a', this).data("name"));

    });
    
    $('.quest-list .single-chapter-section').each(function(){
        var $this = $(this)
